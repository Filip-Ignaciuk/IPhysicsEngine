...
int main() {
  ...
  InitWindow(screenWidth, screenHeight, "IPhysics");
  GuiEnable();
  GuiLoadStyleDefault();
  ...
  geometry->SetMesh(IPhysics::MeshManager::GetMesh("Cube"));
  geometry->SetColor(Color(RED));
  world.AddObject(object);

  /*
  IPhysics::ParticleWorld particleWorld(100,10);
  IPhysics::Particle* particle = new IPhysics::Particle(high, 0.5f, 1.0f);
  particleWorld.GetParticles().push_back(particle);
  IPhysics::ParticleGravity* particleGravity = new
  IPhysics::ParticleGravity(IPhysics::GravityEarth); IPhysics::Firework*
  firework = new IPhysics::Firework(); IPhysics::FireworkManager*
  fireworkManager = new IPhysics::FireworkManager();


  particleWorld.GetParticleForceRegistry().Add(particle, particleGravity);
  IPhysics::ParticleGroundContactGenerator* particleGroundContactGenerator = new
  IPhysics::ParticleGroundContactGenerator();
  ...
  */
  ...
  while (!WindowShouldClose()) {
    ...
    BeginMode3D(camera);
    /*
    IPhysics::ParticleWorld::Particles::iterator iterator =
    particleWorld.GetParticles().begin(); while (iterator !=
    particleWorld.GetParticles().end()){ iPosition = (*iterator)->GetPosition();
        Vector3 position = {iPosition.GetX(), iPosition.GetY(),
    iPosition.GetZ()}; DrawSphere(position, 1.0f, RED);
        ++iterator;
    }
    */
    ...
    while (iterator != world.GetObjects().end()) {
      ...
      Vector4 quaternion{iQuaternion.i, iQuaternion.j, iQuaternion.k,
                         iQuaternion.r};
    ...
    /*
    for (IPhysics::Firework* firework = fireworks; firework < fireworks +
    IPhysics::FireworkManager::GetMaxFireworks(); firework++){ if
    (firework->GetType() == 0){ continue;
        }
        IPhysics::Vector3 iPosition = firework->GetPosition();
        IPhysics::Vector3 iVelocity = firework->GetVelocity();
    ...
    */
    ...
    if (showAddObjectBox) {
      ...
      showAddObjectBox = !GuiWindowBox(
          box,
          IPhysics::LanguageManager::GetText("addobjectmenu.title").c_str());

      GuiSetStyle(LABEL, TEXT_ALIGNMENT_VERTICAL, TEXT_ALIGN_MIDDLE);

      GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_LEFT);

      GuiLabel((Rectangle){box.x + 24, box.y + 24, 96, 24},
               IPhysics::LanguageManager::GetText("addobjectmenu.positiontitle")
                   .c_str());

      GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);

      GuiLabel((Rectangle){box.x + 24, box.y + 48, 24, 24}, "X");
      if (GuiTextBox((Rectangle){box.x + 48, box.y + 48, 96, 24},
      ...
      if (GuiTextBox((Rectangle){box.x + 168, box.y + 48, 96, 24},
      ...
      if (GuiTextBox((Rectangle){box.x + 288, box.y + 48, 96, 24},
      ...
      GuiLabel(
          (Rectangle){box.x + 24, box.y + 72, 96, 24},
          IPhysics::LanguageManager::GetText("addobjectmenu.orientationtitle")
              .c_str());

      GuiSetStyle(LABEL, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);

      GuiLabel((Rectangle){box.x + 24, box.y + 96, 24, 24}, "X");
      if (GuiTextBox((Rectangle){box.x + 48, box.y + 96, 96, 24},
      ...
      if (GuiTextBox((Rectangle){box.x + 168, box.y + 96, 96, 24},
      ...
      if (GuiTextBox((Rectangle){box.x + 288, box.y + 96, 96, 24},
      ...
      GuiLabel((Rectangle){box.x + 24, box.y + 120, 168, 24},
               IPhysics::LanguageManager::GetText("addobjectmenu.masstitle")
                   .c_str());

      if (GuiTextBox((Rectangle){box.x + 24, box.y + 144, 168, 24},
                     textBufferMass, 64, isBufferMassEdited)) {
        isBufferMassEdited = !isBufferMassEdited;
      }

      GuiLabel(
          (Rectangle){box.x + 216, box.y + 120, 168, 24},
          IPhysics::LanguageManager::GetText("addobjectmenu.lineardampingtitle")
              .c_str());

      if (GuiTextBox((Rectangle){box.x + 216, box.y + 144, 168, 24},
                     textBufferLinearDamping, 64,
                     isBufferLinearDampingEdited)) {
        isBufferLinearDampingEdited = !isBufferLinearDampingEdited;
      }

      GuiLabel((Rectangle){box.x + 24, box.y + 168, 168, 24},
               IPhysics::LanguageManager::GetText(
                   "addobjectmenu.angulardampingtitle")
                   .c_str());

      if (GuiTextBox((Rectangle){box.x + 24, box.y + 192, 168, 24},
                     textBufferAngularDamping, 64,
                     isBufferAngularDampingEdited)) {
        isBufferAngularDampingEdited = !isBufferAngularDampingEdited;
      }

      std::string dropDownSelection;
      std::vector<std::string> meshStrings =
          IPhysics::MeshManager::GetMeshStrings();
      for (size_t i = 0; i < meshStrings.size() - 1; i++) {
        dropDownSelection = dropDownSelection + meshStrings[i] + ";";
      }

      dropDownSelection =
          dropDownSelection + meshStrings[meshStrings.size() - 1];

      GuiLabel((Rectangle){box.x + 216, box.y + 168, 168, 24},
               IPhysics::LanguageManager::GetText("addobjectmenu.meshtitle")
                   .c_str());

      if (GuiDropdownBox({box.x + 216, box.y + 192, 168, 24},
                         dropDownSelection.c_str(), &dropDownSelectedMesh,
                         isMeshDropDownActive)) {
        isMeshDropDownActive = !isMeshDropDownActive;
      }

      GuiLabel((Rectangle){box.x + 216, box.y + 216, 168, 24},
               IPhysics::LanguageManager::GetText(
                   "addobjectmenu.angulardampingtitle")
                   .c_str());

      if (GuiTextBox((Rectangle){box.x + 216, box.y + 240, 168, 24},
                     textBufferAngularDamping, 64,
                     isBufferAngularDampingEdited)) {
        isBufferAngularDampingEdited = !isBufferAngularDampingEdited;
      }
      ...
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer1InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer2InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer3InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer4InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer5InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer6InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer7InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer8InverseInertiaTensor, 64, isBeingEdited);
      GuiTextBox((Rectangle){ 100, 100, 48, 24 },
      textBuffer9InverseInertiaTensor, 64, isBeingEdited);
      ...
      */
      ...
      // Add Button
      if (GuiButton(
              (Rectangle){box.x + box.width - 72, box.y + box.height - 48, 48,
                          24},
              IPhysics::LanguageManager::GetText("addobjectmenu.addbutton")
                  .c_str())) {
        // Assuming data is valid
        isValidData = true;

        // Converting the char arrays to real values.
        IPhysics::CharBufferResultStore* xCoordinate =
            IPhysics::CharBufferToReal(textBufferXCoordinate);
        IPhysics::CharBufferResultStore* yCoordinate =
            IPhysics::CharBufferToReal(textBufferXCoordinate);
        IPhysics::CharBufferResultStore* zCoordinate =
            IPhysics::CharBufferToReal(textBufferXCoordinate);
        ...
        if (isValidData) {
          ...
          IPhysics::Object* object = new IPhysics::Object();
          IPhysics::RigidBody* rigidbody =
              object->AddComponent<IPhysics::RigidBody>();
          IPhysics::Geometry* geometry =
              object->AddComponent<IPhysics::Geometry>();
      ...
      showAddMeshBox = !GuiWindowBox(
          box, IPhysics::LanguageManager::GetText("addmeshmenu.title").c_str());

      GuiLabel((Rectangle){box.x + 24, box.y + 216, 168, 24},
               IPhysics::LanguageManager::GetText("addobjectmenu.colortitle")
                   .c_str());

      GuiColorPicker({box.x + 24, box.y + 240, 144, 144}, "Pick a color",
                     &color);
    }
    ...
    if (showHelpBox) {
      ...
      DrawText("Press Z to toggle camera control on and off", 946, 610, 10,
               BLACK);
      DrawText("Press Spacebar to toggle physics simulation on and off", 946,
               630, 10, DARKGRAY);
