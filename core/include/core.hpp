#ifndef IPHYSICS_CORE_HPP
#define IPHYSICS_CORE_HPP
#include <random>
...
struct Vector3 {
  ...
  // Static
  static void MakeOrthonormalBasis(Vector3* vectorA, Vector3* vectorB,
                                   Vector3* vectorC);
};
...
class Matrix3 {
 public:
  ...
  Matrix3(real a1, real a2, real a3, real b1, real b2, real b3, real c1,
          real c2, real c3);
  ...
  void SetComponents(const Vector3& componentOne, const Vector3& componentTwo,
  ...
  // Static
  static Matrix3 LinearInterpolate(const Matrix3& startMatrix,
                                   const Matrix3& endMatrix, real proportion);
};

// A 3 x 4 Matrix
// Stores a transformation consisting of both a rotation and a position.
// It assumes that the remaining four elements are (0,0,0,1), producing a
// homogenous matrix.
class Matrix4 {
 public:
  real data[12]{};
  ...
  void SetOrientationAndPos(const Quaternion& quaternion,
...
struct RandomStore {
  ...
  static Vector3 RandomVector3(const Vector3& lowerBound,
                               const Vector3& upperbound);
};
...
struct LocalWorldTransforms {
  ...
  static Vector3 LocalToWorldDirection(const Vector3& local,
                                       const Matrix4& transform);
  static Vector3 WorldToLocalDirection(const Vector3& world,
                                       const Matrix4& transform);
};
