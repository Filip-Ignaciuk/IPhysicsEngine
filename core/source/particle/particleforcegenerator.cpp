...
void IPhysics::ParticleForceRegistry::Add(
    Particle* particle, ParticleForceGenerator* particleForceGenerator) {
...
void IPhysics::ParticleForceRegistry::Remove(
    Particle* particle, ParticleForceGenerator* particleForceGenerator) {
  ParticleForceRegistration particleForceRegistration{};
  particleForceRegistration.particle = particle;
  particleForceRegistration.particleForceGenerator = particleForceGenerator;
  registrations.erase(std::remove(registrations.begin(), registrations.end(),
                                  particleForceRegistration),
                      registrations.end());
}

std::vector<IPhysics::ParticleForceRegistration>
IPhysics::ParticleForceRegistry::GetRegistrations() {
  return &registrations;
}

void IPhysics::ParticleForceRegistry::Clear() { registrations.clear(); }

void IPhysics::ParticleForceRegistry::UpdateForces(real duration) {
  std::vector<ParticleForceRegistration>::iterator iterator =
      registrations.begin();
  while (iterator != registrations.end()) {
    ParticleForceRegistration particleForceRegistery = *iterator;
    particleForceRegistery.particleForceGenerator->UpdateForce(
        particleForceRegistery.particle, duration);
    ++iterator;
  }
}
...
void IPhysics::ParticleDrag::UpdateForce(Particle* particle, real duration) {
  ...
  velocity.Normalise();
  particle->AddForce(velocity * -1 *
                     (k1 * velocity.Magnitude() +
                      k2 * velocity.Magnitude() * velocity.Magnitude()));
}

IPhysics::ParticleRealGravity::ParticleRealGravity(
...
void IPhysics::ParticleRealGravity::UpdateForce(Particle* particle,
                                                real duration) {
  for (unsigned i = 0; i < registrations->size(); i++) {
    ...
    if (registeration.particleForceGenerator == this &&
        registeration.particle != particle) {
      Vector3 displacement =
          particle->GetPosition() - registeration.particle->GetPosition();
      real denominator = displacement.Magnitude() * displacement.Magnitude() *
                         displacement.Magnitude();
      Vector3 force =
          displacement * (-1 * gravitationalConstant * particle->GetMass() *
                          registeration.particle->GetMass() / denominator);
      particle->AddForce(force);
    }
  }
}

IPhysics::ParticleSpring::ParticleSpring(Particle* otherParticle,
...
IPhysics::ParticleAnchoredSpring::ParticleAnchoredSpring(
    Vector3 anchoredPosition, real springConstant, real restLength) {
  anchoredPosition = anchoredPosition;
  springConstant = springConstant;
  restLength = restLength;
}

void IPhysics::ParticleAnchoredSpring::UpdateForce(Particle* particle,
                                                   real duration) {
  Vector3 direction;
  Vector3 force;
...
IPhysics::ParticleBungee::ParticleBungee(Particle* otherParticle,
                                         real springConstant, real restLength) {
  otherParticle = otherParticle;
  springConstant = springConstant;
  restLength = restLength;
...
IPhysics::ParticleAnchoredBungee::ParticleAnchoredBungee(
    Vector3 anchoredPosition, real springConstant, real restLength) {
  anchoredPosition = anchoredPosition;
  springConstant = springConstant;
  restLength = restLength;
...
void IPhysics::ParticleAnchoredBungee::UpdateForce(Particle* particle,
                                                   real duration) {
  Vector3 direction;
  Vector3 force;
  direction = particle->GetPosition() - anchoredPosition;
...
IPhysics::ParticleBuoyancy::ParticleBuoyancy(real maxDepth, real volume,
                                             real waterHeight,
                                             real liquidDensity) {
  maxDepth = maxDepth;
  volume = volume;
...
void IPhysics::ParticleBuoyancy::UpdateForce(Particle* particle,
                                             real duration) {
  ...
  if (depth <= waterHeight - maxDepth) {
    ...
    particle->AddForce(
        Vector3(0,
                ((depth - maxDepth - waterHeight) / 2 * maxDepth) * volume *
                    liquidDensity,
                0));
  }
}

IPhysics::ParticleFakeAnchoredSpring::ParticleFakeAnchoredSpring(
    Vector3 anchoredPosition, real springConstant, real damping) {
  anchoredPosition = anchoredPosition;
  springConstant = springConstant;
  damping = damping;
}

void IPhysics::ParticleFakeAnchoredSpring::UpdateForce(Particle* particle,
                                                       real duration) {
  ...
  Vector3 target =
      position * RealCos(gamma * duration) + c * RealSin(gamma * duration);
