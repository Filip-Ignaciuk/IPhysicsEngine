...
void IPhysics::ParticleContact::ResolveVelocity(real duration) {
  ...
  real accelerationCausedSeparatedVelocity =
      accelerationCausedVelocity * contactNormal * duration;
  ...
  particles[0]->SetVelocity(particles[0]->GetVelocity() +
                            impulsePerInverseMass *
                                particles[0]->GetInverseMass());
  if (particles[1]) {
    particles[1]->SetVelocity(particles[1]->GetVelocity() +
                              impulsePerInverseMass *
                                  -particles[1]->GetInverseMass());
  }
}
...
IPhysics::ParticleContactResolver::ParticleContactResolver(unsigned iterations)
...
void IPhysics::ParticleContactResolver::ResolveContacts(
    ParticleContact* contactArray, unsigned numberOfContacts, real duration) {
  unsigned i;
  iterationsUsed = 0;
  ...
  while (iterationsUsed < iterations) {
    ...
    for (int i = 0; i < numberOfContacts; i++) {
      ...
      if (separateVelocity > max && separateVelocity < 0 ||
          contactArray[i].penetration > 0) {
