...
IPhysics::real IPhysics::IntersectionTests::TransformToAxis(const CollisionBox& box, const Vector3& axis){
    return box.halfSize.x * RealAbs(axis * box.GetAxis(0)) + box.halfSize.y * RealAbs(axis * box.GetAxis(1)) + box.halfSize.z * RealAbs(axis * box.GetAxis(2));
}

bool IPhysics::IntersectionTests::OverlapOnAxis(const CollisionBox& box1, const CollisionBox& box2, const Vector3& axis, const Vector3& toCentre){
...
bool IPhysics::IntersectionTests::BoxAndHalfSpace(const CollisionBox& box, const CollisionPlane& plane){
    real projectedRadius = TransformToAxis(box, plane.normal);
    real boxDistance = plane.normal * box.GetAxis(3) - projectedRadius;
    return boxDistance  <= plane.offset;
}

bool IPhysics::IntersectionTests::BoxAndBox(const CollisionBox& box1, const CollisionBox& box2){
    ...
    return OverlapOnAxis(box1, box2, box1.GetAxis(0), toCentre) &&
       ...
       OverlapOnAxis(box1, box2, box1.GetAxis(0) % box2.GetAxis(0), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(0) % box2.GetAxis(1), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(0) % box2.GetAxis(2), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(1) % box2.GetAxis(0), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(1) % box2.GetAxis(1), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(1) % box2.GetAxis(2), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(2) % box2.GetAxis(0), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(2) % box2.GetAxis(1), toCentre) &&
       OverlapOnAxis(box1, box2, box1.GetAxis(2) % box2.GetAxis(2), toCentre);
}

unsigned IPhysics::CollisionDetector::SphereAndSphere(const CollisionSphere& firstSphere, const CollisionSphere& secondSphere, CollisionData* data){
    ...
    contact->SetBodyData(firstSphere.rigidbody, secondSphere.rigidbody, data->friction, data->restitution);
    data->AddContacts(1);
    return 1;

}

unsigned IPhysics::CollisionDetector::SphereAndHalfSpace(const CollisionSphere& sphere, const CollisionPlane& plane, CollisionData* data){
    if(data->contactsLeft <= 0){
        return 0;
    }
...
unsigned IPhysics::CollisionDetector::SphereAndPlane(const CollisionSphere& sphere, const CollisionPlane& plane, CollisionData* data){
    if(data->contactsLeft <= 0){
        return 0;
    }
    Vector3 position = sphere.GetAxis(3);
...
unsigned IPhysics::CollisionDetector::BoxAndHalfSpace(const CollisionBox& box, const CollisionPlane& plane, CollisionData* data){
    if(data->contactsLeft <= 0){
        return 0;
    }
...
unsigned IPhysics::CollisionDetector::BoxAndSphere(const CollisionBox& box, const CollisionSphere& sphere, CollisionData* data){
...
#define CHECK_OVERLAP(axis, index) \
    if (!TryAxis(box1, box2, (axis), toCentre, (index), penetration, best)) return 0;

unsigned IPhysics::CollisionDetector::BoxAndBox(const CollisionBox& box1, const CollisionBox& box2, CollisionData* data){
    Vector3 toCentre = box2.GetAxis(3) - box1.GetAxis(3);
    ...
    if (best < 3){
        ...
        FillPointFaceBoxBox(box2, box1, toCentre * -1.0f, data, best - 3, penetration);
        data->AddContacts(1);
        return 1;
    }
    else{
        ...
        Vector3 vertex = ContactPoint(pointOnOneEdge, oneAxis, box1.halfSize[oneAxisIndex], pointOnTwoEdge, twoAxis, box2.halfSize[twoAxisIndex], bestSingleAxis > 2);
        ...
        contact->SetBodyData(box2.rigidbody, box2.rigidbody, data->friction, data->restitution);
        data->AddContacts(1);
        return 1;
    }
    return 0;
...
bool IPhysics::CollisionDetector::TryAxis(const CollisionBox& box1, const CollisionBox& box2, Vector3 axis, const Vector3& toCentre, unsigned index, real& smallestPenetration, unsigned& smallestCase){
...
IPhysics::real IPhysics::CollisionDetector::PenetrationOnAxis(const CollisionBox& box1, const CollisionBox& box2, const Vector3& axis, const Vector3& toCentre){
...
void IPhysics::CollisionDetector::FillPointFaceBoxBox(const CollisionBox& box1, const CollisionBox& box2, const Vector3& toCentre, CollisionData* data, unsigned best, real penetration){
    ...
    contact->SetBodyData(box1.rigidbody, box2.rigidbody, data->friction, data->restitution);
}

IPhysics::Vector3 IPhysics::CollisionDetector::ContactPoint(const Vector3& pointOnOneEdge, const Vector3& oneAxis, real oneSize, const Vector3& pointOnTwoEdge, const Vector3& twoAxis, real twoSize, bool useOne){
