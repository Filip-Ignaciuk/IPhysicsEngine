...
void IPhysics::ForceRegistry::Add(
...
void IPhysics::ForceRegistry::Remove(
    Object* object, const std::shared_ptr<ForceGenerator>& forceGenerator) {
  ForceRegistration forceRegistration{};
  forceRegistration.rigidBody = object->GetComponent<RigidBody>();
  forceRegistration.forceGenerator = forceGenerator;
...
void IPhysics::ForceRegistry::RemoveAll() { registrations.clear(); }

const IPhysics::ForceRegistration* IPhysics::ForceRegistry::Get(
    Object* object) const {
...
void IPhysics::ForceRegistry::Clear() { registrations.clear(); }
...
void IPhysics::ForceRegistry::UpdateForces(real duration) {
  ...
  while (iterator != registrations.end()) {
    ...
    forceRegistration.forceGenerator->UpdateForce(forceRegistration.rigidBody,
                                                  duration);
    ++iterator;
  }
}
